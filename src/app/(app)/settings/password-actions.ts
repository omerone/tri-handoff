...
import { consumeRateLimit, deleteOtherUserSessions, setPasswordHash } from '@/lib/db';
...
export async function changePasswordAction(
   ...
   */
  const strength = validatePasswordStrength(next, [session.user.email]);
  if (!strength.isStrong) {
    return { error: strength.feedback.warning || t('passwordWeak') };
  }

  await setPasswordHash(session.user.id, await hashPassword(next));
