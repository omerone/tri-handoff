 ...
 * did crypto and indices do" is one question about two asset classes, and a dropdown that took
 * one forced it to be asked twice and the numbers added up by hand — which the summary bar
 * exists to do and could not, because it only ever saw one of the two.
 *
 ...
 * value that no phone has. This is a button and a panel of ordinary checkboxes: tapping is the
 * whole interaction, on either device.
 *
 * Nothing is applied until the panel closes. The alternative — a navigation per tick — turns
 * choosing three classes into three page loads, each re-rendering the table underneath the
 * panel the trader is still reading, and leaves the back button holding two states nobody
 * meant to visit.
 *
 * **The "2 chosen" label is built here, not handed in.** An earlier version took a
 * `(count) => string` from the server component that owns the labels, which is a function
 * crossing the RSC boundary: React cannot serialise one, so the *page* threw while encoding
 * its own props and every render of `/trades` in production returned a server error. A count
 * is client state and a message with a plural in it has to be built where the number is —
 * which is the same reason `selection.tsx` reaches for `useTranslations` rather than being
 * given its sentences.
 */
export function MultiFilter({
   ...
   *
   * Kept in a ref so the listener below is not rebuilt every time a box is ticked — and, more
   * importantly, so it always reads the latest draft rather than the one captured when the
    ...
    <div className="relative flex min-w-0 flex-col gap-1" ref={box}>
      ...
      {open ? (
        <div
          ...
          aria-label={name}
          /*
            `z-30` and absolute: the panel sits over the table below it, and the filter bar is a
            flex row whose height must not grow when one of its children opens — the summary
            figures beside it would jump down the page every time a filter was pressed.
          */
        ...
        >
          {/* Clearing is in the same panel, so undoing a filter is where making it was. */}
          <button
            type="button"
            onClick={() => setDraft([])}
            ...
            return (
              ...
              >
                <input
                  ...
                  className="sr-only"
                />
                {/* Drawn rather than a native box, so the tick lines up with the row's text at
                    this size on every platform instead of sitting a pixel high on one. */}
                <span
                  aria-hidden
